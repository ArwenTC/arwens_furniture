...
execute if block ^ ^ ^-.1 #af.base:replaceable run setblock ^ ^ ^-.1 light[level=11]
...
tag @e[tag=af.furniture_interaction,tag=af.new,type=interaction,distance=..10] add af.wall_candle_interaction