...
execute as @e[tag=af.plant_display,distance=..8] if score @s af.furniture_id = @e[tag=af.selected,type=interaction,distance=..7,limit=1] af.furniture_id run tag @s add af.selected_plant

# If the display has a plant, remove the plant.
execute as @e[tag=af.selected_plant,distance=..10,tag=af.has_plant] at @s run function af.base:furniture/action/plant/has_plant
